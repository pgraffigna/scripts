...
read -pr "$(echo -e "${amarillo}"Ingresa el ID para la VM \>\>  "${end}")" ID
read -pr "$(echo -e "${amarillo}"Ingresa el nombre de la cloud-image \>\>  "${end}")" IMAGEN
read -pr "$(echo -e "${amarillo}"Ingresa el nombre para la VM \>\>  "${end}")" NOMBRE_VM
read -pr "$(echo -e "${amarillo}"Ingresa la cantidad de RAM \>\>  "${end}")" RAM
